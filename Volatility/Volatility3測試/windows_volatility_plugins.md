...
| Plugin | 功能說明 | 用途 |
|---|---|---|
| `windows.info.Info` | 顯示記憶體映像檔的 Windows 系統與核心資訊 | 確認 OS 版本、Kernel、Symbol 是否正確 |
...
# 最常用的 Windows Plugin 建議優先學

如果要做實作，不需要全部 Plugin 都做，建議先學下面這幾個：

```bash
vol.exe -f memory.vmem windows.info.Info
vol.exe -f memory.vmem windows.pslist.PsList
vol.exe -f memory.vmem windows.pstree.PsTree
vol.exe -f memory.vmem windows.psscan.PsScan
vol.exe -f memory.vmem windows.cmdline.CmdLine
...
vol.exe -f memory.vmem windows.malfind.Malfind --pid <PID>
vol.exe -f memory.vmem windows.filescan.FileScan
vol.exe -f memory.vmem windows.registry.hivelist.HiveList
vol.exe -f memory.vmem windows.registry.userassist.UserAssist
```
