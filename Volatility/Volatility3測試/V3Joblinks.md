...
本次使用 `windows.joblinks.JobLinks` 分析 Process 與 Windows Job Object 的關聯。

結果顯示，大部分 Job 關係屬於正常程式行為，例如 Chrome 子行程與 WMI 行程。較重要的發現是 `Rick And Morty` 與 `vmware-tray.exe` 出現在同一組 Job Object 中。
...
`vmware-tray.exe` 的路徑為：

```text
C:\Users\Rick\AppData\Local\Temp\RarSFX0\vmware-tray.exe
```

這代表 `Rick And Morty` 與 `vmware-tray.exe` 不只存在父子關係，也在 Job Object 層級被關聯在一起。綜合前面 `pstree`、`cmdline`、`sessions` 與 `envars` 的結果，可以判斷這兩個 Process 屬於同一條可疑執行鏈。
