...
而 `SSDT` Plugin 主要檢查 Kernel 層級是否遭 Hook。

本次結果沒有發現可疑 SSDT Hook，因此可判斷本案目前較不像是 Kernel Rootkit 或 Driver 層攻擊，而是使用者執行可疑 EXE 後造成的 User-mode 感染與加密行為。

---

## 8. 結論

`windows.ssdt.SSDT` 成功列出系統中的 SSDT 項目。

本次結果中，大多數 SSDT 項目皆指向 Windows 正常核心模組：

```text
ntoskrnl
```

未發現明顯指向未知 Driver、可疑模組或惡意 Hook 的 SSDT 項目。

因此，此 Plugin 結果可作為排除證據，表示目前沒有發現 SSDT Hook 或 Kernel Rootkit 層級攻擊跡象。
